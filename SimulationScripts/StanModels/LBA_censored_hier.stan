...
data{
  ...
  int N_subjects;
  int N;
  array[N_subjects] matrix[N,2] RT;
}

parameters {
  real<lower=0> bMinusA_group;
  real<lower=0> A_group;
  real<lower=0> tau_group;
  vector<lower=0>[NUM_CHOICES] v_group;
  vector<lower=0>[N_subjects] bMinusA_subject;
  vector<lower=0>[N_subjects] A_subject;
  vector<lower=0,upper=.2>[N_subjects] tau_subject;
  matrix<lower=0>[NUM_CHOICES, N_subjects] v_subject;
}
...
model {
  bMinusA_group ~ normal(.4,.4)T[0,];
  A_group ~ normal(1,.5)T[0,];
  tau_group ~ normal(0,.1)T[0,.2];
  for(n in 1:NUM_CHOICES) { 
    v_group[n] ~ normal(1,.1)T[0,];
  }

  for (sn in 1:N_subjects) { 
    bMinusA_subject[sn] ~ normal(bMinusA_group,.4)T[0,];
    A_subject[sn] ~ normal(A_group, .5)T[0,];
    tau_subject[sn] ~ normal(tau_group,.1)T[0,.2];
    for(n in 1:NUM_CHOICES) { 
      v_subject[n] ~ normal(v_group[n],.1)T[0,];
    }
    RT[sn] ~ lba(bMinusA_subject[sn],A_subject[sn],v_subject[,sn],s,tau_subject[sn]);
  }
}
